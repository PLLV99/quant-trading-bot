...
class RiskManager:
    ...
    def __init__(self, config):
        ...
        self.martingale_detection_enabled = config.get(
        ...
        if self.ftmo_mode:
            print(
                f"[RiskManager] FTMO MODE ACTIVE. Tightening Drawdown to {self.max_drawdown_ftmo_pct*100}%"
            )
    ...
    def reset_daily_stats(self, current_balance):
        ...
        self.daily_trading_halted = False
        print(
            f"[RiskManager] DAILY RESET. Start Balance: ${self.daily_start_balance:.2f}"
        )
    ...
    def check_daily_status(self, current_balance):
        ...
        if daily_pnl_pct <= -self.daily_loss_limit_pct:
            if not self.daily_trading_halted:
                print(
                    f"[RISK HALT] Daily Loss Limit Hit ({daily_pnl_pct*100:.2f}%). HALTING TRADES."
                )
        ...
        if daily_pnl_pct >= self.daily_profit_target_pct:
            if not self.daily_trading_halted:
                print(
                    f"[RISK HALT] Daily Profit Target Hit ({daily_pnl_pct*100:.2f}%). HALTING TO PRESERVE GAINS."
                )
                self.daily_trading_halted = True
        ...
        if self.peak_balance > 0:
            ...
            if self.current_drawdown > (self.max_drawdown_limit * 0.8):
                ...
                if not self.circuit_breaker_active:
                    print(
                        f"[RISK ALERT] Drawdown {drawdown*100:.2f}% detected. Activating CIRCUIT BREAKER."
                    )
                ...
                if self.circuit_breaker_active:
                    print(
                        f"[RISK RESTORE] Drawdown recovered to {drawdown*100:.2f}%. Deactivating Circuit Breaker."
                    )
                    self.circuit_breaker_active = False
    ...
    def calculate_position_size(
        self, account_balance: float, current_volatility_atr: float, price: float
    ) -> float:
        """
        ...
        if streak_mult < 1.0:
            print(
                f"[DEFENSE] Losing streak active: Reducing size to {streak_mult*100:.0f}%"
            )
        ...
        if self.martingale_detection_enabled:
            ...
            if is_martingale:
                print(
                    f"[RISK HALT] Martingale detected. Resetting to minimum safe lot."
                )
    ...
    def check_trade_allowed(self, signal_type: str, price: float) -> bool:
        ...
        if self.current_drawdown >= self.max_drawdown_limit:
            print(
                f"[KILL SWITCH] Max Drawdown ({self.current_drawdown*100:.2f}%) Exceeded. Trade REJECTED."
            )
            return False
    ...
    def detect_martingale(self, new_lot_size: float) -> bool:
        ...
        Returns:
            True if dangerous Martingale pattern detected (2+ consecutive increasing lots)
        """
    ...
    def update_floating_loss(
        self, unrealized_pnl: float, account_balance: float
    ) -> bool:
        """
        ...
        if account_balance > 0:
            ...
            if unrealized_pnl < 0 and floating_pct > self.floating_loss_limit_pct:
                if not self.floating_loss_cb_active:
                    print(
                        f"[RISK ALERT] ⚠️ Floating Loss {floating_pct*100:.2f}% exceeds limit ({self.floating_loss_limit_pct*100}%)"
                    )
    ...
    def record_trade_result(self, is_win: bool):
        ...
        if is_win:
            if self.losing_streak_count >= self.losing_streak_threshold:
                print(
                    f"[RISK RESTORE] Win after {self.losing_streak_count} losses. Resetting streak."
                )
            self.losing_streak_count = 0
        else:
            ...
            if self.losing_streak_count >= self.losing_streak_threshold:
                print(
                    f"[RISK ALERT] ⚠️ Losing streak: {self.losing_streak_count}. Reducing risk by {(1-self.losing_streak_reduction)*100}%."
                )
    ...
    def get_volatility_multiplier(self, current_atr: float) -> float:
        ...
        """
        if (
            not self.volatility_scaling_enabled
            or self.normal_atr is None
            or self.normal_atr == 0
        ):
            return 1.0
        ...
        if volatility_ratio > 1.5:  # High volatility (50%+ above normal)
            print(
                f"[DEFENSE] High volatility ({volatility_ratio:.2f}x normal): Halving size"
            )
            return 0.5
